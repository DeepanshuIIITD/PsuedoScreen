...
//   // TODO: Top performers will come from backend API in future
...
//   // Filter students based on search query only (no top performers logic)
//   const filteredList = studentNameList
...
//   // TODO: Top performers logic - will be implemented when backend API is ready
...
//         {/* TODO: Top Performers Button - Will be enabled when backend API is ready */}
//         {/* <TouchableOpacity
...
// Hardcoded streak data keyed by student ID (will be replaced by backend data in future)
// Format: { [studentId]: streakCount }
const HARDCODED_STREAKS = {};
    ...
    try {
      ...
      if (data?.students) {
        setStudentNameList(data.students.map(s => ({
          ...
          firstName: s.FirstName,
          lastName: s.LastName,
          streak_count: s.currentStreak ?? HARDCODED_STREAKS[s.ID.toString()] ?? 0,
          // ↑ Uncomment above and remove line below once backend sends streak_count
          // streak_count: HARDCODED_STREAKS[s.ID.toString()] ?? Math.floor(Math.random() * 15),
        })));
      }
    } catch (e) {
      console.error("Error fetching students:", e);
      Alert.alert('Error', 'Unable to load students. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error', 'Unable to remove student. Please try again.');
            }
          }
        }
      ]
      ...
      <View style={[styles.studentCard, { backgroundColor: colorScheme === 'dark' ? '#111827' : '#f3f4f6' }]}>
        ...
        <View style={styles.studentInfo}>
          ...
          <View style={styles.streakRow}>
            ...
            <Text style={[styles.streakText, { color: palette.sub }]}>
